...
public class ExecuteElementProcessing implements ElementProcessing {
    ...
    private List<WebElement> getElementsForMagicWait(By locator) {
        try {
            ...
            if (StringUtils.containsIgnoreCase(typeError.getStackTrace().toString(),"TypeError: can't access dead object")) {
                driver.switchTo().defaultContent();
                return driver.findElements(locator);
            } else throw new TestingBlazeRunTimeException("The last frame might be dead and the context is still trying to get it");
        }
    }
