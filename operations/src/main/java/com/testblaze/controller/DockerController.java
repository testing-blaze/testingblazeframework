...
public final class DockerController {
    public static void startDocker() {
        if (!isAlive()) {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Trying to start Docker container " + getContainerId());
            runCommand("docker start " + getContainerId());

            long startTime = System.currentTimeMillis() / 1000;
            while (!isAlive() && (startTime + 15 > System.currentTimeMillis() / 1000)) {
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Waiting for Docker container " + getContainerId() + " to start");
            }
        } else {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Docker container " + getContainerId() + " is already started");
        }
    }

    public static void stopDocker() {
        if (isAlive()) {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Trying to stop Docker container " + getContainerId());
            runCommand("docker stop " + getContainerId());

            long startTime = System.currentTimeMillis() / 1000;
            while (isAlive() && (startTime + 15 > System.currentTimeMillis() / 1000)) {
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Waiting for Docker container " + getContainerId() + " to stop");
            }
        } else {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, "Docker container " + getContainerId() + " is already stopped");
        }
    }
    ...
    private static boolean readCommandResult(Process proc) {
        ...
        try {
            ...
            while ((s = stdError.readLine()) != null) {
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, s);
                result = false;
            }
        } catch (IOException e) {
            e.printStackTrace();
