 ...
 * under the License.
 */
package com.testblaze.actionsfactory.abstracts;
...
import static com.testblaze.misclib.ConsoleFormatter.COLOR.BRIGHT_BLUE;
import static com.testblaze.misclib.ConsoleFormatter.COLOR.BRIGHT_CYAN;
import static com.testblaze.misclib.ConsoleFormatter.ICON.WEB;
...
public interface PageLoadProcessing<T> {
    ...
    PageLoadProcessing<String> windowLoad = (reportMessage) -> {
        ...
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BRIGHT_BLUE, WEB, "Level 2 -> page Load status "+reportMessage+" is: " + setBoldText(pageLoadStatus)+" in "+(Math.round(reportEndTime)*10)/10.0+" seconds");
    };
    ...
    PageLoadProcessing<String> documentLoad = (reportMessage) -> {
        ...
        double reportEndTime = (System.currentTimeMillis() / 1000.0) - reportStartTime;
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BRIGHT_CYAN, WEB, "Level 1 -> page Load status "+reportMessage+" is: " + setBoldText(pageLoadStatus)+" in "+(Math.round(reportEndTime)*10)/10.0+" seconds");
    };
}
