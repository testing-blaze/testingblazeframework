...
import static com.testblaze.misclib.ConsoleFormatter.COLOR.PURPLE;
import static com.testblaze.misclib.ConsoleFormatter.ICON.*;
...
public class ExecuteElementProcessing implements ElementProcessing {
    ...
    public WebElement forSingleElement(By locator) {
        ...
        isViewPort(element);
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, CHECKERED_FLAG, "Element Processing Ends");
    ...
    public List<WebElement> forListOfElements(By locator) {
        ...
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, CHECKERED_FLAG, "List of Elements Processing Ends");
    ...
    private <T> List<WebElement> listOfElementsWaitProcessing(T locatorOrElement) {
        ...
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, THUMBS_UP, String.format("List of Elements Presence Check Completed in %.1f seconds", reportEndTime));
        return listOfElements;
    }
    ...
    private <T> WebElement elementWaitProcessing(T locatorOrElement) {
        ...
        if (locatorOrElement instanceof By) {
                    ...
                    try {
                        if (isElementDrawn(ele)) {
                            ...
                            if (ele.isDisplayed()) {
                                ...
                                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_IMPORTANT, THUMBS_UP, "Element is Displayed & Enabled on page");
                            }
                            break;
                        }
                        ...
                        if (magicWaitRetry == 0) {
                            magicWaitRetry++;
                            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, THUMBS_DOWN, "Element is stale so re-trying one more time");
            ...
            if (element == null) {
                try {
                    ...
                    i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_ERROR, THUMBS_DOWN, "Element does not exist in DOM");
                    throw e;
                }
            }
        ...
        while ((elementVisibilityWaitTime > i.amPerforming().waitFor().getCurrentTimeInSecs() && displayedFlag) && !isElementDrawnValidated) {
            try {
                ...
                if (magicWaitRetry == 0) {
                    magicWaitRetry++;
                    i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, "Element is stale so re-trying one more time");
                    elementWaitProcessing(locatorOrElement);
                } else {
                    displayedFlag = false;
                }
        ...
        magicWaitRetry = 0;
        double reportEndTime = (System.currentTimeMillis() / 1000.0) - reportStartTime;
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, THUMBS_UP, String.format("Element Presence/Creation Completed in %.1f seconds", reportEndTime));
        return element;
    }
    ...
    private void completeElementCreationOnUi(WebElement element) {
        ...
        while (i.amPerforming().waitFor().getCurrentTimeInSecs() < waitTime) {
            ...
            if (newSize > iSize) iSize = newSize;
                ...
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_IMPORTANT, TICK, "Element Creation on UI completed");
                break;
            }
        }
    }
    ...
    private void countMatchingNodesOnPage(By locator) {
        ...
        if (nodes != 1) {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, THUMBS_DOWN, "Total element matching nodes in DOM --> " + nodes);
        } else {
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, THUMBS_UP, "Total element matching nodes in DOM --> " + nodes);
        }
    }
    ...
    private void projectProcessingWrapper() {
        ...
        try {
            if (turnOnProcessingHoldOnScreen && (driver.findElements(processingHoldOnScreen).get(0).getRect().getDimension().getWidth() > 0 || driver.findElement(processingHoldOnScreen).isEnabled())) {
                ...
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, PURPLE, HOURGLASS, String.format("Waited for hold on screen to fade away for %s seconds", endTime));
            }
        } catch (Exception e) {
            /* Ignore Exception */
        }
    ...
    private void isViewPort(WebElement element) {
        ...
        try {
            ...
            if (
                ...
                isViewPortCounter = 0;
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, OVERLAPPING_FRAME, "Element ViewPort confirmed");
            } else {
                if (isViewPortCounter < 5) {
                    isViewPortCounter++;
                    isViewPort((element));
                }
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_IMPORTANT, THUMBS_DOWN, "Element ViewPort not confirmed");
            }
        } catch (Exception e) {
