...
import com.testblaze.actionsfactory.processing.ExecuteClickProcessing;
...
import static com.testblaze.misclib.ConsoleFormatter.COLOR.BRIGHT_GREEN;
...
public class ExecuteCoreActions implements Action {
    ...
    public void doIt(WebElement element) {
        ...
        try {
            ...
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BRIGHT_GREEN, ROBOT, "Click action performed successfully");
    ...
    public void doIt(WebElement element, String input) {
        ...
        try {
            ...
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BRIGHT_GREEN, ROBOT, "Input action performed successfully");
        } catch (ElementClickInterceptedException interceptException) {
    ...
    private void retryExecutor(WebElement element, String... args) {
        ...
        i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, RED, X_MARK, args[1] + " : Retrying to "+args[0]+" for maximum " + Waits.STANDARD_WAIT_TIME + " seconds");
        while (timerLimit > System.currentTimeMillis() / 1000) {
            try {
                if (args[0].equalsIgnoreCase("click")) {
                    element.click();
                    i.amPerforming().switchTo().acceptAlert();
                    i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, RED, TICK, args[1] + " : Retrying is Successful " + Waits.STANDARD_WAIT_TIME + " seconds");
                    break;
                ...
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, RED, X_MARK, "Stale Element Exception : Cancelling Retry ");
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_CRITICAL, RED, X_MARK, "Next Step might fail");
                break;
