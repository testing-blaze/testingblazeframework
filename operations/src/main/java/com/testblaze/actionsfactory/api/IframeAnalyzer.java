...
import static com.testblaze.misclib.ConsoleFormatter.COLOR.BLUE;
...
import static com.testblaze.misclib.ConsoleFormatter.ICON.DOLPHIN;
...
public class IframeAnalyzer {
    ...
    private void manageSwitching() {
        for (WebElement iframes : device.getDriver().findElements(IFRAME)) {
            ...
            if (device.getDriver().findElements(locator).size() > 0) {
                i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BRIGHT_CYAN, DOLPHIN, String.format("Successfully Switched to iframe with id '%s'", switchedFrameInfo));
    ...
    private void switchToDefaultContent() {
        try {
            ...
            i.amPerforming().updatingReportWith().write(LogLevel.TEST_BLAZE_INFO, BLUE, CYCLONE, "Default Context Enabled");
        } catch (Exception e) {
        }
